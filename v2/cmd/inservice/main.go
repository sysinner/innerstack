...
func configRefresh(domains []*inapi2.GatewayService_DomainDeploy) error {
	...
	domainFresh := func(domainEntry *DomainEntry, domain *inapi2.GatewayService_DomainDeploy) {
		domainEntry.mu.Lock()
		...
		for _, route := range prevRoutes {
			...
			case "localfs":
				...
				if p := lynkapi.SlicesSearchFunc(domain.Routes, func(a *inapi2.GatewayService_DomainDeploy_Route) bool {
